...
import { ArrowLeft, Calendar, Tag, Download, Crown, Shield, Sparkles } from "lucide-react";
...
const VIPContentDetails = () => {
    ...
    const fetchContentDetails = async () => {
      try {
        ...
        const response = await axios.get<{ data: string }>(
          ...
          { headers: { Authorization: `Bearer ${token}`, "x-api-key": `${import.meta.env.VITE_FRONTEND_API_KEY}` } }
        );
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 flex items-center justify-center p-4">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="max-w-md bg-gray-800/90 backdrop-blur-xl border border-gray-700 rounded-2xl p-8 text-center shadow-2xl">
          ...
          <Link to="/vip" className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-yellow-500 to-yellow-600 hover:from-yellow-600 hover:to-yellow-700 text-black rounded-xl font-semibold transition-all duration-300">
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 flex items-center justify-center p-4">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="max-w-md bg-gray-800/90 backdrop-blur-xl border border-gray-700 rounded-2xl p-8 text-center shadow-2xl">
          ...
          <p className="text-gray-300 mb-6">The VIP content you're looking for doesn't exist or has been removed.</p>
          <Link to="/vip" className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-yellow-500 to-yellow-600 hover:from-yellow-600 hover:to-yellow-700 text-black rounded-xl font-semibold transition-all duration-300">
            <ArrowLeft className="w-4 h-4" />
            Back to VIP content
          </Link>
  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 overflow-x-clip">
      <Helmet>
        <title>Sevenxleaks - {content.name} (VIP)</title>
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute top-1/4 left-1/2 -translate-x-1/2 w-64 h-64 sm:w-96 sm:h-96 bg-yellow-500/10 rounded-full blur-3xl animate-pulse" />
        <div className="absolute bottom-1/4 left-1/2 -translate-x-1/2 w-64 h-64 sm:w-96 sm:h-96 bg-yellow-500/10 rounded-full blur-3xl animate-pulse" />
      </div>

      <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Back Button */}
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="mb-6">
          <Link to="/vip" className="inline-flex items-center gap-2 px-4 py-2 bg-gray-800/60 hover:bg-gray-700/80 border border-gray-700 hover:border-yellow-500/50 rounded-xl text-gray-300 hover:text-white transition-all duration-300 backdrop-blur-sm shadow-lg hover:shadow-yellow-500/10">
            <ArrowLeft className="w-4 h-4" />
        ...
        {/* VIP Content Card */}
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="bg-gray-800/90 backdrop-blur-xl border border-yellow-500/30 rounded-2xl overflow-hidden shadow-2xl shadow-yellow-500/10">
          {/* VIP Header */}
          <div className="bg-gradient-to-r from-yellow-900/40 to-yellow-800/40 px-6 py-6 border-b border-yellow-500/20">
            ...
            <div className="flex flex-wrap items-center gap-2 sm:gap-4">
              ...
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="flex items-center gap-2 px-2 sm:px-3 py-1 sm:py-1.5 bg-yellow-500/20 text-yellow-300 rounded-lg border border-yellow-500/30 backdrop-blur-sm">
          ...
          <div className="p-6">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="mb-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h2 className="text-xl font-bold text-white">VIP Download Options</h2>
              </div>
              ...
              <div className="bg-gradient-to-r from-yellow-500/10 to-yellow-600/10 border border-yellow-500/20 rounded-xl p-4 mb-4">
                ...
                <p className="text-gray-300 text-sm">Enjoy instant access, no ads, and premium download speeds as a VIP member.</p>
              </div>
            </motion.div>
