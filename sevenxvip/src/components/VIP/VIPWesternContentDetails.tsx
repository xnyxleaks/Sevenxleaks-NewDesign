...
const VIPWesternContentDetails = () => {
  ...
  useEffect(() => {
    const fetchContentDetails = async () => {
      try {
        ...
        console.error("Error fetching VIP Western content details:", error);
      ...
      <div>
        <div className="min-h-screen bg-gradient-to-br from-gray-900 via-yellow-900/10 to-gray-900 flex items-center justify-center p-4">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="max-w-md bg-gray-800/90 backdrop-blur-xl border border-yellow-500/30 rounded-2xl p-8 text-center shadow-2xl">
            ...
            <Link to="/vip-western" className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-yellow-500 to-yellow-600 hover:from-yellow-600 hover:to-yellow-700 text-black rounded-xl font-semibold transition-all duration-300">
      ...
      <div>
        ...
        <div className="min-h-screen bg-gradient-to-br from-gray-900 via-yellow-900/10 to-gray-900 flex items-center justify-center p-4">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="max-w-md bg-gray-800/90 backdrop-blur-xl border border-yellow-500/30 rounded-2xl p-8 text-center shadow-2xl">
            ...
            <p className="text-gray-300 mb-6">The VIP Western content you're looking for doesn't exist or has been removed.</p>
            <Link to="/vip-western" className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-yellow-500 to-yellow-600 hover:from-yellow-600 hover:to-yellow-700 text-black rounded-xl font-semibold transition-all duration-300">
              <ArrowLeft className="w-4 h-4" />
              Back to VIP Western
            </Link>
  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-br from-gray-900 via-yellow-900/10 to-gray-900 overflow-x-clip">
      <Helmet>
        <title>Sevenxleaks - {content.name} (VIP Western)</title>
        <link rel="canonical" href={`https://sevenxleaks.com/vip-western/${content.slug}`} />
        <style>{`
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute bottom-1/4 left-1/2 -translate-x-1/2 w-64 h-64 sm:w-96 sm:h-96 bg-orange-500/10 rounded-full blur-3xl animate-pulse" />
      </div>

      <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Back Button */}
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="mb-6">
          <Link to="/vip-western" className="inline-flex items-center gap-2 px-4 py-2 bg-gray-800/60 hover:bg-gray-700/80 border border-yellow-500/30 hover:border-yellow-400/50 rounded-xl text-gray-300 hover:text-white transition-all duration-300 backdrop-blur-sm shadow-lg hover:shadow-yellow-500/10">
            <ArrowLeft className="w-4 h-4" />
            <span className="text-sm">Back to VIP Western</span>
          </Link>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="bg-gray-800/90 backdrop-blur-xl border border-yellow-500/40 rounded-2xl overflow-hidden shadow-2xl shadow-yellow-500/20">
          {/* VIP Header */}
          <div className="bg-gradient-to-r from-yellow-900/50 to-orange-900/30 px-6 py-6 border-b border-yellow-500/30">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div className="flex items-center gap-2 px-3 py-1 bg-gradient-to-r from-yellow-500/30 to-orange-500/20 text-yellow-300 rounded-full border border-yellow-500/40 backdrop-blur-sm">
                <Globe className="w-3 h-3" />
                <span className="font-bold text-xs hidden sm:inline">VIP WESTERN EXCLUSIVE</span>
                <span className="font-bold text-xs sm:hidden">VIP WESTERN</span>
              </div>
            </div>
          ...
          <div className="p-6">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="mb-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h2 className="text-xl font-bold text-white">VIP Western Download Options</h2>
                <Star className="w-5 h-5 text-yellow-400 animate-pulse" />
              </div>

              <div className="bg-gradient-to-r from-yellow-500/15 to-orange-500/10 border border-yellow-500/30 rounded-xl p-4 mb-4">
                <div className="flex items-center gap-2 mb-2">
                  <Crown className="w-4 h-4 text-yellow-400" />
                  <span className="text-yellow-400 font-semibold text-sm">VIP Western Benefits Active</span>
                  <Globe className="w-4 h-4 text-orange-400" />
                </div>
                <p className="text-gray-300 text-sm">Enjoy instant access to premium Western content, no ads, and exclusive download speeds as a VIP member.</p>
              </div>
            </motion.div>
...
export default VIPWesternContentDetails;